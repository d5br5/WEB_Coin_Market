...
export const authentication = async (req, res, next) => {
    ...
    if (!authorization) {
        return res.status(200).json({ok: false, error: "no authorized"});
    }
    const [bearer, key] = authorization.split(" ");
    if (bearer !== "Bearer") {
        return res.status(200).json({ok: false, error: "no bearer key"});
    }
    req.token = key;
    let decodedPK = "";
    try {
    ...
    try {
        ...
        return res.status(200).send({ok: false, error: "token error"});
    }
