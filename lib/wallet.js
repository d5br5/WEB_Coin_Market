...
const wallet = async function (req, res) {
    ...
    const document = {
        ...
        btc: await Coin.find({ code: 'bitcoin' }),
        bch: await Coin.find({ code: 'bitcoin-cash' }),
        xrp: await Coin.find({ code: 'ripple' }),
        eos: await Coin.find({ code: 'eos' }),
        eth: await Coin.find({ code: 'ethereum' }),
        trx: await Coin.find({ code: 'tron' })
    }
